...
func main() {
	...
	if Config.Dev.RequestLog {
		...
		if Config.Dev.RequestLogCounter {
			...
			router.Use(func(c *gin.Context) {
				...
				log.Printf("[middleware][RequestLog] | [%d] %s %s - %s ? %s", DEV_request_counter, c.Request.RemoteAddr, c.Request.Method, c.Request.URL.Path, query)
				c.Next()
			})
		} else {
			router.Use(func(c *gin.Context) {
				query := c.Request.URL.Query()
				log.Printf("[middleware][RequestLog] | %s %s - %s ? %s", c.Request.RemoteAddr, c.Request.Method, c.Request.URL.Path, query)
				c.Next()
			})
		}
	}
	...
	GetCache(router)
	RedirectToCacheServerForFontawesome(router)
	GetStatic(router)
